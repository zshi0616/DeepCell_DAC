...
class ECOTrainer():
    def __init__(self,
                 args, 
                 model, 
                 loss_weight = [1.0, 1.0, 2.0], 
                 device = 'cpu', 
                 distributed = False
                 ):
        super(ECOTrainer, self).__init__()
        # Config
        self.args = args
    ...
    def run_batch(self, batch):
        mcm_pm_tokens, mask_indices, pm_tokens, pm_prob, pred_drive, gt_drive = self.model(batch)
        ...
        # Task 3: Drive Prediction
        drive_loss = self.clf_loss(pred_drive, gt_drive.float().unsqueeze(1))
        
        loss_status = {
            'prob_loss': prob_loss,
            'mcm_loss': mcm_loss, 
            'drive_loss': drive_loss, 
        }
        return loss_status
    ...
    def train(self, num_epoch, train_dataset, val_dataset):
        ...
        prob_loss_stats, mcm_loss_stats, drive_loss_stats = AverageMeter(), AverageMeter(), AverageMeter()
            ...
            for phase in ['train', 'val']:
                ...
                for iter_id, batch in enumerate(dataset):
                    ...
                    loss = loss_status['prob_loss'] * self.loss_weight[0] + \
                        ...
                        loss_status['drive_loss'] * self.loss_weight[2]
                    ...
                    drive_loss_stats.update(loss_status['drive_loss'].item())
                    if self.local_rank == 0:
                        Bar.suffix = '[{:}/{:}]|Tot: {total:} |ETA: {eta:} '.format(iter_id, len(dataset), total=bar.elapsed_td, eta=bar.eta_td)
                        Bar.suffix += '|Prob: {:.4f} |MCM: {:.4f} |Dve: {:.4f} '.format(prob_loss_stats.avg, mcm_loss_stats.avg, drive_loss_stats.avg)
                ...
                if self.local_rank == 0:
                    self.logger.write('{}| Epoch: {:}/{:} |Prob: {:.4f} |MCM: {:.4f} |Dve: {:.4f} |Net: {:.2f}s\n'.format(
                        phase, epoch, num_epoch, prob_loss_stats.avg, mcm_loss_stats.avg, drive_loss_stats.avg, batch_time.avg))
                    bar.finish()
